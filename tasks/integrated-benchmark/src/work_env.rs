...
impl WorkEnv {
    ...
    fn build(&self) {
        ...
        // Share one cargo target dir across every per-revision build.
        // Cargo's fingerprint cache reuses compiled artifacts whose
        // source path, content, and build flags match. External
        // dependencies are fetched into `~/.cargo/registry` at a
        // single shared path, so two revisions with the same
        // `Cargo.lock` entry for an external crate produce the same
        // fingerprint and the second build reuses the first build's
        // `.rlib`. Pacquet's internal crates live at per-revision
        // paths, so their fingerprints differ and they still rebuild,
        // but the external dependency rebuild dominates a fresh
        // release build and is what this sharing skips.
        let shared_target = self.root().join(".shared-target");
        fs::create_dir_all(&shared_target).expect("create shared cargo target dir");
        ...
        for revision in self.revision_names() {
            ...
            // Capture this revision's binary into the per-revision
            // tree before the next build overwrites the shared
            // output. The `install.bash` generated by `init()`
            // expects `./pacquet/target/release/pacquet` relative to
            // the bench dir. `fs::copy` overwrites the destination,
            // so no explicit remove is needed when the file already
            // exists from a cache restore.
            let revision_release_dir = revision_repo.join("target").join("release");
            fs::create_dir_all(&revision_release_dir).expect("create per-revision target/release");
            fs::copy(
                shared_target.join("release").join("pacquet"),
                revision_release_dir.join("pacquet"),
            )
            .expect("copy pacquet binary into per-revision dir");
        }
    }
    ...
    fn benchmark(&self) {
        ...
        // Hyperfine pairs repeated `--prepare` flags 1:1 with the
        // positional commands, so each benchmark command gets its
        // own `rm -rf` scoped to that command's bench dir. A single
        // shared `--prepare` would wipe every revision's
        // `node_modules` and `store-dir` before every iteration,
        // making each transition between commands pay a full
        // install's worth of `rm -rf` on the previous command's
        // freshly populated tree before the next command's first
        // warmup. On a slow CI runner that overhead can push the
        // step over its timeout even when the timed runs themselves
        // succeeded.
        //
        // Per-iteration cleanup paths come from the scenario.
        // Cold-cache scenarios wipe `node_modules` and `store-dir`.
        // Hot-cache wipes only `node_modules` so the warmup-populated
        // store survives into the timed runs.
