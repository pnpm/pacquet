...
impl WorkEnv {
    ...
    fn build(&self) {
        ...
        for revision in self.revision_names() {
            ...
            // the bench dir.
            let dest_dir = revision_repo.join("target").join("release");
            fs::create_dir_all(&dest_dir).expect("create per-revision target/release");
            let src = shared_target.join("release").join("pacquet");
            let dest = dest_dir.join("pacquet");
            if dest.exists() {
                fs::remove_file(&dest).expect("remove old per-revision pacquet binary");
            }
            fs::copy(&src, &dest).expect("copy pacquet binary into per-revision dir");
        }
    }
